...
function AlbumCarousel({ albums, itemsPerView = 'responsive' }) {
  ...
  }, [itemsPerView, albums.length]);

  const maxIndex = Math.max(0, albums.length - itemsToShow);
  ...
  if (itemsPerView === 'all' || albums.length <= itemsToShow) {
    return (
